...
def config_Neighbours(xv,yv,FFA,neighbours):
      ...
      if yv < 5 and FFA[yv + 1][xv] != 0:
            neighbours.append((yv+1,xv))
      if xv > 0 and FFA[yv][xv - 1] != 0:
            neighbours.append((yv,xv-1))
      if xv < 5 and FFA[yv][xv + 1] != 0:
            neighbours.append((yv,xv+1))
      return neighbours
...
def stack_Update(current_value,md,xv,yv,yp,xp,FFA,neighbours):
      ...
      if (current_value-1!=md):
            ...
            if (len(neighbours)!=0):
                  ...
                  north_Value = FFA[yv-1][xv] if (config_Maze[yp-1][xp] == " ") else 100
                  south_Value = FFA[yv+1][xv] if (config_Maze[yp+1][xp] == " ") else 100
                  west_Value = FFA[yv][xv-1] if (config_Maze[yp][xp-1] == " ") else 100
                  east_Value = FFA[yv][xv+1] if (config_Maze[yp][xp+1] == " ") else 100
